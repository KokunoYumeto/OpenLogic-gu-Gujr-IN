...
BODY_PATH = ROOT / "build" / "first-order-introduction-body.tex"
...
def find_body_line(target: dict[str, Any], decision_id: str) -> tuple[int, str] | None:
    ...
    if decision_id.startswith(("OLFUN-", "OLSIZ-", "OLARI-", "OLINF-", "OLPL-", "OLPRF-", "OLSEQ-", "OLND-", "OLTAB-", "OLAX-", "OLCO-", "OLFINT-")):
...
def synctex_page(body_line: int) -> int | None:
    command = [
        ...
        "view",
        "-i",
        f"{body_line}:1:build/first-order-introduction-body.tex",
        "-o",
        "build/gu-first-order-introduction.pdf",
    ]
        ...
        for occurrence in decision["occurrences"]:
            ...
            if page is not None:
                occurrence["reader_locator"] = {
                    ...
                    "profile": "cumulative Gujarati reader through OLP-0148; first-order introduction after completeness",
                    "printed_page": None,
                    "assembled_pdf_page": page,
                    "provenance": (
                        f"SyncTeX forward query at build/first-order-introduction-body.tex:{body_line}; "
...
def markdown_full(decisions: list[dict[str, Any]], generated: str) -> str:
    lines = [
        ...
        "",
        f"Generated {generated}. This register covers all {len(decisions)} material decisions recorded for the current 145/722-unit working edition. Each occurrence has exact source and Gujarati line and UTF-8 byte evidence. Reader pages are reported only when deterministic alignment and SyncTeX agree; unresolved pages remain explicitly pending.",
        "",
    ]
    for decision in decisions:
        lines.extend(
...
This directory is the review entry point for the current 145/722-unit Gujarati
working edition.
