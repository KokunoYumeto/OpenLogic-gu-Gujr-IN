...
BODY_PATH = ROOT / "build" / "tableaux-body.tex"
PDF_PATH = ROOT / "build" / "gu-tableaux.pdf"
SYNCTEX_PATH = ROOT / "build" / "gu-tableaux.synctex.gz"
BUILD_RECEIPT_PATH = ROOT / "build" / "BUILD_RECEIPT_011.json"
BT = chr(96)
...
def find_body_line(target: dict[str, Any], decision_id: str) -> tuple[int, str] | None:
    ...
    if decision_id.startswith(("OLFUN-", "OLSIZ-", "OLARI-", "OLINF-", "OLPL-", "OLPRF-", "OLSEQ-", "OLND-", "OLTAB-")):
...
def synctex_page(body_line: int) -> int | None:
    command = [
        ...
        "view",
        "-i",
        f"{body_line}:1:build/tableaux-body.tex",
        "-o",
        ...
        for occurrence in decision["occurrences"]:
            ...
            if page is not None:
                occurrence["reader_locator"] = {
                    ...
                    "profile": "cumulative Gujarati reader through OLP-0111; signed analytic first-order tableaux with identity",
                    "printed_page": None,
                    "assembled_pdf_page": page,
                    "provenance": (
                        f"SyncTeX forward query at build/tableaux-body.tex:{body_line}; "
...
def markdown_full(decisions: list[dict[str, Any]], generated: str) -> str:
    lines = [
        ...
        "",
        f"Generated {generated}. This register covers all {len(decisions)} material decisions recorded for the current 108/722-unit working edition. Each occurrence has exact source and Gujarati line and UTF-8 byte evidence. Reader pages are reported only when deterministic alignment and SyncTeX agree; unresolved pages remain explicitly pending.",
        "",
    ]
    for decision in decisions:
        lines.extend(
...
This directory is the review entry point for the current 108/722-unit Gujarati
working edition.
