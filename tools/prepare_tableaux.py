"""Prepare the cumulative Gujarati reader through signed analytic tableaux.

This adapter launches no TeX process. It extends the natural-deduction reader
with the complete classical first-order tableaux chapter, selects the frozen
source's FOL and tableaux profile, and records the translated
...
BUILD = ROOT / "build"
scope = runpy.run_path(str(ROOT / "tools" / "prepare_natural_deduction.py"))
body = (BUILD / "natural-deduction-body.tex").read_text(encoding="utf-8")
receipts = json.loads((BUILD / "natural-deduction-input-hashes.json").read_text(encoding="utf-8"))
...
def expand_tokens(text: str) -> str:
    def replacement(match: re.Match[str]) -> str:
        ...
        assert suffix in {"", "s", "d", "x"}, (key, suffix)
        if suffix == "d":
            assert key == "derive"
            return tokens[key][0]
        if suffix == "x":
            assert key == "tableau"
        return tokens[key][suffix in {"s", "x"}]
...
active_tags.update({"FOL", "prfTab", "prvAll", "prvEx"})
...
chapter_root = ROOT / "gu" / "content" / "first-order-logic" / "tableaux"
driver = chapter_root / "tableaux.tex"
...
(BUILD / "tableaux-body.tex").write_text(body, encoding="utf-8", newline="\n")
(BUILD / "tableaux-available-labels.tex").write_text(
    "\n".join(
...
(BUILD / "tableaux-available-labels.json").write_text(
    json.dumps(sorted(keys), indent=2) + "\n",
    encoding="utf-8",
    newline="\n",
)
(BUILD / "tableaux-input-hashes.json").write_text(
    json.dumps(receipts, ensure_ascii=False, indent=2) + "\n",
    encoding="utf-8",
    newline="\n",
)

assert len(receipts) == 104, len(receipts)
sections = len(re.findall(r"\\olfileid\{", body))
assert sections == 96, sections
...
print(json.dumps({
    ...
    "source_units_covered": 108,
    "input_receipts": len(receipts),
    "labels": len(keys),
    "logic_profile": "classical_first_order_signed_analytic_tableaux",
