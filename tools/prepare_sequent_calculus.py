"""Prepare the cumulative Gujarati reader through first-order sequent calculus.

This adapter launches no TeX process. It extends the proof-systems reader with
the complete classical first-order LK chapter, selects the frozen source's FOL
and quantifier-rule profile, and records the translated chapter driver as a
covered source unit.
"""
...
BUILD = ROOT / "build"
scope = runpy.run_path(str(ROOT / "tools" / "prepare_proof_systems.py"))
body = (BUILD / "proof-systems-body.tex").read_text(encoding="utf-8")
receipts = json.loads((BUILD / "proof-systems-input-hashes.json").read_text(encoding="utf-8"))
...
active_tags.update({"FOL", "prfLK", "prvAll", "prvEx"})
...
chapter_root = ROOT / "gu" / "content" / "first-order-logic" / "sequent-calculus"
driver = chapter_root / "sequent-calculus.tex"
...
(BUILD / "sequent-calculus-body.tex").write_text(body, encoding="utf-8", newline="\n")
(BUILD / "sequent-calculus-available-labels.tex").write_text(
    "\n".join(
...
(BUILD / "sequent-calculus-available-labels.json").write_text(
    json.dumps(sorted(keys), indent=2) + "\n",
    encoding="utf-8",
    newline="\n",
)
(BUILD / "sequent-calculus-input-hashes.json").write_text(
    json.dumps(receipts, ensure_ascii=False, indent=2) + "\n",
    encoding="utf-8",
    newline="\n",
)

assert len(receipts) == 76, len(receipts)
sections = len(re.findall(r"\\olfileid\{", body))
assert sections == 70, sections
...
print(json.dumps({
    ...
    "source_units_covered": 80,
    "input_receipts": len(receipts),
    "labels": len(keys),
    "logic_profile": "classical_first_order_LK",
