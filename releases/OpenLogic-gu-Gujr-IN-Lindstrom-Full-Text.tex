% OpenLogic Gujarati v0.16.0 cumulative full-text source.
% This file materializes the exact 204-unit reader body, label index, and base driver.
% External OpenLogic styles, fonts, and diagram assets remain in the companion source ZIP.
% Generated from the anonymously verified lindstrom-v0.16.0 checkpoint.
% Materialization changes packaging only; it does not change translation wording.

\def\gueditiontitle{લિન્ડસ્ટ્રોમનું પ્રમેય સહિત ઓપન લોજિક ગુજરાતી}
\def\gueditionheading{%
  ...
  નિદર્શસિદ્ધાંતના પાયા, અંકગણિતના નિદર્શો, અંતર્વેશન પ્રમેય તથા
  લિન્ડસ્ટ્રોમનું પ્રમેય}
\def\gueditiondescription{%
  આ આવૃત્તિમાં અગાઉના પ્રકરણો સાથે લિન્ડસ્ટ્રોમના પ્રમેયનું સંપૂર્ણ
  પ્રકરણ ઉમેર્યું છે. તેમાં અમૂર્ત તર્કશાસ્ત્રો, સામાન્યતાના ગુણધર્મો,
  સઘનતા, અધોગામી લેવેનહાઇમ--સ્કોલેમ, આંશિક એકરૂપતા અને પ્રથમ-ક્રમ
  તર્કશાસ્ત્રના અભિવ્યક્તિશીલ મહત્તમત્વની સાબિતી સામેલ છે.}
\def\gueditioncount{૨૦૪}
\newcommand{\guincludesyntaxbibliography}{}
\def\gueditionbodyfile{build/lindstrom-body.tex}
\def\gueditionlabelsfile{build/lindstrom-available-labels.tex}
\def\gueditioncorrectionids{%
  ...
  OLMAR-001થી OLMAR-016, OLINT-001થી OLINT-010 અને
  OLLIN-001થી OLLIN-013 સુધીની}
\def\gueditioneditorial{%
  લિન્ડસ્ટ્રોમના નવા પ્રકરણમાં અમૂર્ત તર્કશાસ્ત્ર, સામાન્યતા, સઘનતા,
  અધોગામી લેવેનહાઇમ--સ્કોલેમ અને પ્રથમ-ક્રમ તર્કશાસ્ત્રના મહત્તમત્વની
  સાબિતી સામેલ છે. સ્થિર મૂળની તેર ઓળખેલી પાઠ્ય, ઔપચારિક અથવા અર્થલક્ષી
...
\expandafter\def\csname guavailable@mod:lin:alg:sec\endcsname{1}
\expandafter\def\csname guavailable@mod:lin:int:sec\endcsname{1}
\expandafter\def\csname guavailable@mod:lin:lsp:sec\endcsname{1}
\expandafter\def\csname guavailable@mod:lin:lsp:thm:abstract-p-isom\endcsname{1}
\expandafter\def\csname guavailable@mod:lin:prf:lem:lindstrom\endcsname{1}
\expandafter\def\csname guavailable@mod:lin:prf:sec\endcsname{1}
\expandafter\def\csname guavailable@mod:lin:prf:thm:lindstrom\endcsname{1}
...
\tableofcontents
\clearpage
% BEGIN MATERIALIZED 204-unit cumulative Gujarati body
\part{ગણો}
...
\section{લિન્ડસ્ટ્રોમનું પ્રમેય}
...
\subsection{અમૂર્ત તર્કશાસ્ત્રો}
...
ચલો $\alpha$, $\beta$,~\dots વાપરીએ છીએ અને સામાન્ય પ્રથમ-ક્રમ
સૂત્રો માટે $\varphi$, $\psi$,~\dots અનામત રાખીએ છીએ.

\begin{defn}
  વર્ગ $\Setabs{\Struct{M}}{\Struct{M} \models_L \alpha}$ને
  $\Mod(L){\alpha}$થી દર્શાવીએ. ભાષા સ્પષ્ટ કરવી જરૂરી હોય તો
  $\Mod[L](L){\alpha}$ લખીએ. $\Lang{L}$ માટેની બે સંરચનાઓ
...
\item (\emph{વિસ્તાર ગુણધર્મ}) દરેક $\alpha \in L(\Lang{L})$ માટે
  $\Lang{L}$નો એવો \emph{સાન્ત} ઉપગણ $\Lang{L'}$ છે કે
  $\Struct{M} \models_L \alpha$ સંબંધ $\Struct{M}$ના $\Lang{L'}$ પરના
  ન્યૂનરૂપ પર જ આધાર રાખે; એટલે કે, $\Struct{M}$ અને $\Struct{N}$નાં
  $\Lang{L'}$ પરનાં ન્યૂનરૂપો સરખાં હોય, તો $\Struct{M} \models_L \alpha$
  ત્યારે અને માત્ર ત્યારે જ્યારે $\Struct{N} \models_L \alpha$.
\item (\emph{એકરૂપતા ગુણધર્મ}) જો $\Struct{M} \models_L \alpha$ અને
  $\Struct{M} \simeq \Struct{N}$, તો $\Struct{N} \models_L \alpha$ પણ.
\item (\emph{પુનઃનામકરણ ગુણધર્મ}) $\models_L$ સંબંધ પુનઃનામકરણ હેઠળ
  ...
  વાક્ય~$\alpha$ માટે $\Struct{M} \models_L \alpha$ ત્યારે અને માત્ર
  ત્યારે જ્યારે $\Struct{M}' \models_L \alpha'$, જ્યાં $\Struct{M}'$ એ
  $\Struct{M}$ને અનુરૂપ $\Lang{L}'$-સંરચના છે અને $\alpha' \in
  L(\Lang{L}')$.
...
\item (\emph{બૂલીય ગુણધર્મ}) અમૂર્ત તર્કશાસ્ત્ર
  ...
  $\alpha \in L(\Lang{L})$ માટે એવું $\beta \in L(\Lang{L})$ છે કે
  $\Struct{M} \models_L \beta$ ત્યારે અને માત્ર ત્યારે જ્યારે
  $\Struct{M} \not\models_L \alpha$; અને દરેક $\alpha$ તથા $\beta$ માટે એવું
  $\gamma$ છે કે $\Mod(L){\gamma} = \Mod(L){\alpha} \cap \Mod(L){\beta}$. આણ્વિક
  સૂત્રો અને બીજા સંયોજકો માટે પણ એ જ રીતે.
\item (\emph{પરિમાણક ગુણધર્મ}) $\Lang{L}$ના દરેક અચળ $c$ અને
  $\alpha \in L(\Lang{L})$ માટે એવું $\beta \in L(\Lang{L'})$ છે કે
  \[
  \Mod[L'](L){\beta} = \Setabs{\Struct{M}}{\Expan{M}{a} \in
  \Mod[L](L){\alpha} \text{ કોઈક } a \in \Domain{M}},
  \]
  ...
  \sourcecorrection{OLLIN-003}{સ્થિર અંગ્રેજી મૂળની
    \texttt{abstract-logics.tex}, પંક્તિઓ~80--87માં $\beta$ને
    $L(\Lang L)$માં મૂકવામાં આવ્યું છે, પરંતુ એની નિદર્શ-શ્રેણી
    $\Lang L'=\Lang L\setminus\{c\}$ની સંરચનાઓની છે. તેથી $\beta$ એ
    $L(\Lang L')$નું વાક્ય હોવું જોઈએ; અહીં આ પ્રકાર સુધાર્યો છે.}
\item (\emph{સાપેક્ષીકરણ ગુણધર્મ}) વાક્ય $\alpha \in
  L(\Lang{L})$ અને $\Lang{L}$માં ન હોય એવા સંકેતો $R$, $c_1$,
  \dots, $c_n$ આપેલા હોય ત્યારે એવું વાક્ય $\beta \in
  L(\Lang{L} \cup \{R,c_1,\ldots,c_n\})$ છે, જેને $\alpha$નું
  $\Atom{R}{x, c_1, \dots c_n}$ને \emph{સાપેક્ષીકરણ} કહીએ છીએ, કે
  દરેક સંરચના~$\Struct{M}$ માટે:
  \[
  \Expan{M}{X, b_1, \ldots, b_n} \models_L \beta \text{ ત્યારે અને
    માત્ર ત્યારે જ્યારે } \Struct{N} \models_L \alpha,
  \]
...
$\Lang{L}$ અને વાક્ય $\alpha \in L_1(\Lang{L})$ માટે એવું
વાક્ય $\beta \in L_2(\Lang{L})$ હોય કે
$\Mod[L](L_1){\alpha} = \Mod[L](L_2){\beta}$, તો બીજું તર્કશાસ્ત્ર પહેલા
...
\begin{rem}
  ...
  અને વાક્ય $\alpha$નું $\Atom{R}{x, c_1, \dots, c_n}$ને
  સાપેક્ષીકરણ દરેક ઉપસૂત્ર $\lforall[x][\beta]$ને
  $\lforall[x][(\Atom{R}{x, c_1, \dots, c_n} \lif \ \beta)]$થી બદલીને
...
\subsection{સઘનતા અને લેવેનહાઇમ--સ્કોલેમ ગુણધર્મો}
...
તર્કશાસ્ત્રો સુધી ચાલતી નથી, કારણ કે મનસ્વી $\alpha \in L(\Lang{L})$ માટે
...
\begin{proof}
ધારો કે $\Struct{M} \simeq_p \Struct{N}$, પણ કોઈ $\alpha$ માટે
$\Struct{M} \models_L \alpha$ જ્યારે $\Struct{N} \not\models_L \alpha$ પણ છે.
...
ભાષા~$\Lang{L}$ માટે $\alpha \in L(\Lang{L})$. $\Struct{M}$ અને
...
$\Struct{A}$ને એવી સંરચના ગણો જેનું વ્યાપકક્ષેત્ર
$\Struct{M}^*$ અને $\Struct{N}^*$નાં વ્યાપકક્ષેત્રોનો સંઘ છે, જેમાં
$\Struct{M}^*$ અને $\Struct{N}^*$ ઉપસંરચનાઓ છે, અને જેની
ભાષામાં એક વધારાનું દ્વિસ્થાની વિધેય~$R$ છે જેનું
અર્થઘટન સંબંધ~$I$ છે તથા વ્યાપકક્ષેત્રો $\Domain{M}^*$ અને $\Domain{N}^*$ દર્શાવતાં
વિધેયો છે.
...
એવું $L$-વાક્ય $\theta_1$ છે જે $\Struct{A}$માં સાચું છે અને કહે છે
કે $\Struct{M} \models_L \alpha$ તથા $\Struct{N} \not\models_L \alpha$ (આમાં
સાપેક્ષીકરણ અને બૂલીય ગુણધર્મો જરૂરી છે); અને એવું પ્રથમ-ક્રમ
વાક્ય $\theta_2$ પણ છે જે $\Struct{A}$માં સાચું છે અને કહે છે કે
આંશિક એકરૂપતા~$I$ દ્વારા $\Struct{M} \simeq_p \Struct{N}$.
\sourcecorrection{OLLIN-007}{સ્થિર અંગ્રેજી મૂળની
  \texttt{ls-property.tex}, પંક્તિઓ~109--115માં $\theta_1$ને પ્રથમ-ક્રમ
  વાક્ય કહેવામાં આવ્યું છે. મનસ્વી અમૂર્ત વાક્ય $\alpha$નો આંતરિક
  સંરચનાઓ પર સંતોષ સાપેક્ષીકરણથી $L$-વાક્ય તરીકે વ્યક્ત થાય છે;
  $\theta_2$ પ્રથમ-ક્રમ વાક્ય છે અને સામાન્યતા પરથી $L$માં પણ આવે છે.
  અહીં આ પ્રકારભેદ સ્પષ્ટ કર્યો છે.}
લેવેનહાઇમ--સ્કોલેમ ગુણધર્મથી $\theta_1$ અને $\theta_2$ કોઈ
ગણનીય નિદર્શ $\Struct{A}_0$માં એકસાથે સાચાં છે. તેમાં એવી
આંશિક રીતે એકરૂપ ઉપસંરચનાઓ $\Struct{M}_0$ અને $\Struct{N}_0$ છે કે
$\Struct{M}_0 \models_L \alpha$ અને $\Struct{N}_0 \not\models_L \alpha$.
...
\subsection{લિન્ડસ્ટ્રોમનું પ્રમેય}

\begin{lem}
\ollabel{lem:lindstrom}
ધારો કે $\Lang{L}$ સાન્ત છે, $\alpha \in L(\Lang{L})$, અને એવું
$n \in \Nat$ પણ છે કે કોઈ પણ બે સંરચનાઓ $\Struct{M}$ અને
$\Struct{N}$ માટે, જો $\Struct{M} \equiv_n \Struct{N}$ અને
$\Struct{M} \models_L \alpha$, તો $\Struct{N} \models_L \alpha$ પણ. ત્યારે
$\alpha$ કોઈ પ્રથમ-ક્રમ વાક્યને સમકક્ષ છે; એટલે કે, એવું પ્રથમ-ક્રમ
$\theta$ છે કે $\Mod(L){\alpha} = \Mod(L){\theta}$.
\end{lem}

\begin{proof}
એવું $n$ લો કે કોઈ પણ બે $n$-સમકક્ષ સંરચનાઓ $\Struct{M}$ અને
$\Struct{N}$, $\alpha$ને આપાતા સત્યમૂલ્ય પર સંમત હોય.
...
સંરચના~$\Struct{M}$ માટે $\theta_{\Struct{M}}$ને $\Struct{M}$માં
સાચાં અને $\QuantRank{\varphi} \le n$ ધરાવતાં બધાં પ્રથમ-ક્રમ
વાક્યો~$\varphi$નું સંયોજન ગણો (તાર્કિક સમકક્ષતા સુધી આ સંયોજન
સાન્ત છે), જેથી $\Struct{N} \models \theta_{\Struct{M}}$ ત્યારે અને માત્ર
ત્યારે જ્યારે $\Struct{N} \equiv_n \Struct{M}$. પછી
$\theta = \textstyle\bigvee
\Setabs{\theta_{\Struct{M}}}{\Struct{M} \models_L \alpha}$ મૂકો; આ વિયોજન પણ
...
\sourcecorrection{OLLIN-009}{સ્થિર અંગ્રેજી મૂળની
  ...
  $\alpha$ જ ફરીથી સંયોજનમાં વ્યાપ્ત પ્રથમ-ક્રમ વાક્ય માટે વપરાય છે. અંદરના
  બંધ ચલને $\varphi$ કરીને નિશ્ચિત $\alpha$થી અલગ રાખ્યો છે.}

$\Mod(L){\alpha} = \Mod(L){\theta}$નો નિષ્કર્ષ હવે મળે છે. ખરેખર, જો
$\Struct{N} \models_L \theta$, તો કોઈ $\Struct{M} \models_L \alpha$ માટે
$\Struct{N} \models \theta_{\Struct{M}}$; તેથી લેમાની પરિકલ્પનાથી
$\Struct{N} \models_L \alpha$ પણ. ઊલટું, જો $\Struct{N} \models_L \alpha$,
તો $\theta_\Struct{N}$ એ $\theta$માંનો એક વિયોજ્ય છે, અને $\Struct{N}
\models \theta_\Struct{N}$ હોવાથી $\Struct{N} \models_L \theta$ પણ.
\end{proof}
...
$\Lang{L}$ માટેના દરેક $\alpha \in L(\Lang{L})$ માટે એવું $n \in \Nat$
છે કે કોઈ પણ બે સંરચનાઓ $\Struct{M}$ અને $\Struct{N}$ માટે: જો
$\Struct{M} \equiv_n \Struct{N}$, તો $\Struct{M}$ અને $\Struct{N}$
$\alpha$ પર સંમત હોય. કારણ કે ત્યારે $\alpha$ પ્રથમ-ક્રમ વાક્યને સમકક્ષ
...
$\alpha$ આપેલું હોય ત્યારે વિરોધાભાસ માટે ધારો કે દરેક $n$ માટે એવી
સંરચનાઓ $\Struct{M}_n$ અને $\Struct{N}_n$ છે કે
$I_n(\emptyset, \emptyset)$, પણ (ધારો કે) $\Struct{M}_n \models_L \alpha$
જ્યારે $\Struct{N}_n \not\models_L \alpha$. એકરૂપતા ગુણધર્મથી આપણે માની
...
$\Domain{M} \cup \Domain{M}^{<\omega}$ વ્યાપકક્ષેત્ર ધરાવતો અને જોડાણ
વિધેયો $P$ અને $Q$ ધરાવતી વિસ્તૃત ભાષામાંનો
$\Struct{M}$નો વિસ્તાર ગણો.

$\Struct{N}_n$, $\Struct{N}$ અને $\Struct{N}^*$ને એ જ રીતે વ્યાખ્યાયિત
કરો. હવે $\Struct{A}$ને એવી સંરચના ગણો જેના વ્યાપકક્ષેત્રમાં
$\Struct{M}^*$ અને $\Struct{N}^*$નાં વ્યાપકક્ષેત્રો ઉપરાંત પ્રાકૃતિક
સંખ્યાઓ~$\Nat$ તથા તેમનો સ્વાભાવિક ક્રમ~$\le$ આવે છે. એની
ભાષામાં વ્યાપકક્ષેત્રો $\Domain{M}$, $\Domain{N}$,
...
વાક્ય~$\theta$ છે જે કહે છે કે $\le$ એ પ્રથમ પરંતુ અંતિમ ઘટક વિનાનો
વિકિર્ણ સુરેખ ક્રમ છે, $\Struct{M}_n \models_L \alpha$ અને
$\Struct{N}_n \not\models_L \alpha$, અને ક્રમના દરેક $n$ માટે
$J(n, \mathbf{a}, \mathbf{b})$ ત્યારે અને માત્ર ત્યારે જ્યારે
$I_n(\mathbf{a}, \mathbf{b})$.
\sourcecorrection{OLLIN-012}{સ્થિર અંગ્રેજી મૂળની
  \texttt{lindstrom-proof.tex}, પંક્તિઓ~100--105માં અમૂર્ત વાક્ય
  $\alpha$ માટે $\models$ લખાયું છે. $\alpha\in L(\Lang L)$ હોવાથી અહીં
  સંતોષ-સંબંધ $\models_L$ અને તેનો નિષેધ જરૂરી છે; બંને મૂક્યાં છે.}

એક નવું અચળ $c$ ઉમેરો અને દરેક પ્રમાણભૂત $m\in\Nat$ માટે
$\psi_m(c)$ને એવું પ્રથમ-ક્રમ વાક્ય ગણો જે કહે છે કે ક્રમમાં
$c$ પહેલાં ઓછામાં ઓછા $m$ ઘટકો છે. ગણ
\[
  \{\theta\} \cup \Setabs{\psi_m(c)}{m\in\Nat}
\]
...
\sourcecorrection{OLLIN-013}{સ્થિર અંગ્રેજી મૂળની
  \texttt{lindstrom-proof.tex}, પંક્તિઓ~107--108માં એકલા $\theta$ પર
  સઘનતા લાગુ કરીને અપ્રમાણભૂત ઘટકવાળું નિદર્શ મળતું હોવાનું કહેવાયું
  છે. $\theta$નું પ્રમાણભૂત પ્રાકૃતિક-ક્રમવાળું નિદર્શ પણ છે, તેથી એ
...
$\Struct{M_{n^*}} \models_L \alpha$ અને $\Struct{N_{n^*}}
\not\models_L \alpha$. હવે $\Domain{M_{n^*}}$ અને
...
\end{proof}
% END MATERIALIZED 204-unit cumulative Gujarati body
\clearpage
